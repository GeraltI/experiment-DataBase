...
int writeBlockSizeToDisk(unsigned char *blkPtr, unsigned int addr, Buffer *buf, int Size)
{
    char filename[40];
    unsigned char *bytePtr;
    ...
    for (bytePtr = blkPtr; bytePtr < blkPtr + Size; bytePtr++)
        fputc((int)(*bytePtr), fp);
...
int writeBlockAppendToDisk(unsigned char *blkPtr, unsigned int addr, Buffer *buf)
{
    char filename[40];
    unsigned char *bytePtr;
